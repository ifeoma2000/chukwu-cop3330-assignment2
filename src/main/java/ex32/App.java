...
public class App {
    public static void main(String[] args){
        ...
        if (difficulty.equals("1") || difficulty.equals("2") || difficulty.equals("3"))
            notValid = false;
        while(!difficulty.matches("-?\\d+(\\.\\d+)?") || notValid)
        {
            System.out.println("Please input a valid difficulty.");
            difficulty = input.next();
            if (difficulty.equals("1") || difficulty.equals("2") || difficulty.equals("3"))
                notValid = false;
        }

        int rand = genNum(difficulty);

        System.out.print("I have my number. What's your guess?");
        String guess = input.next();
        int numGuess = 1;
        while(Integer.parseInt(guess) != rand)
        {
        ...
        System.out.printf("You got it in %d guesses!", numGuess);

    }
    public static int genNum(String selection)
    {
        int rand;
